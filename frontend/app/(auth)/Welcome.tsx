    ...
    <SafeAreaView className='flex h-full items-center justify-between  bg-white'>
        ...
        >
               ...
               <View key={item.id} className='flex items-center justify-center p-5 '>
                    ...
                    <Text className='text-lg text-center font-semibold text-[#858585] mx-10 mt-3'>{item.description}</Text>
               </View>
               ))} 
            
        </Swiper>
        <CustomButton title={isLastSlide? 'Get Started':'Next'} className="w-11/12 mt-100 p-4  "
        onPress={()=>isLastSlide ? router.replace('/(auth)/SignUp')
